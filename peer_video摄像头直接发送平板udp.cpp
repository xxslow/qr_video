...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString) {
  do {
    ...
    //xzq a hack to shutdown strean when stop playing.
...
void DummySink::snd_frame_to_wmh(unsigned f_len)
{
	...
	printf("sendtoWMH: %u %u %u\n", tmp_f_cnt, sum, cnt + 1);
		...
		sendto(s_obu_to_cd_snd_video_fd, snd_buf, head_len + LEN_MAX_PER_FRAME, MSG_DONTWAIT, NULL, 0);
		//usleep(500);
		//msg_type_count++;
	...
	//send left data
	sprintf(snd_buf, "V %u %u %u ", tmp_f_cnt, sum, sum);
	head_len = strlen(snd_buf);
#ifdef DEBUG_PRINT_EACH_RECEIVED_FRAME
	...
	sendto(s_obu_to_cd_snd_video_fd, snd_buf, head_len + left_len, MSG_DONTWAIT, NULL, 0);
	//usleep(500);
	//msg_type_count++;
	//printf("packet_count %d\n",++packet_count);

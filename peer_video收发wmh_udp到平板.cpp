...
//#define DEBUG_NUM
#define MAX_UINT  4294967296  // 4 Byte 最大包编号，超过后断开重传

char eventLoopWatchVariable;	//0:线程启用中;1:关闭线程;2:线程关闭中
static unsigned int s_f_cnt = 0;	//视频帧计数
...
//====================================================================
//静态变量
static struct video_thr s_video_thr;	//保存线程信息
...
// use udp
#define CD_ADDR			"192.168.110.116"
#define CD_PORT			50001
static int s_obu_to_cd_snd_video_fd;
    ...
    switch(type)
    {
        case INFO_WMH_UDM:
        {
			...
			if (tmp_wmh_udm->msg_type == 1){
				memcpy(tx_buf, tmp_wmh_udm->data, tmp_wmh_udm->length);

				sendto(s_obu_to_cd_snd_video_fd, tx_buf, tmp_wmh_udm->length, MSG_DONTWAIT, NULL, 0);
...
int udp_connect(const char *addr, int port)
{
		...
		printf("udp connect addr is %s port is %d\n", addr, port);
        return sockfd;
}
...
int main(int argc, char** argv)
{
	...
	if (argc != 2){
		...
		printf("peer_video 0 for broadcast only\n");
		printf("peer_video 1 for receive only\n");
		printf("peer_video 2 for both broadcast and receive\n");
		goto End;
	}
	else{
		param = argv[1][0];
	...
	if (param == '1' || param == '2'){
		s_obu_to_cd_snd_video_fd = udp_connect(CD_ADDR, CD_PORT);
		if(s_obu_to_cd_snd_video_fd < 0) {
			printf("%s: video udp connect to %s <%d> failed\n", __func__, CD_ADDR, CD_PORT);
			return -1;
		}
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString) {
  do {
	...
	s_my_rtspClient = rtspClient;//xzq a hack to shutdown strean when command 7 is received.
...
void DummySink::snd_frame_to_wmh(unsigned f_len)
{
	...
	// construct wmh udm wireless type
	char tx_buf[MAX_BUF_LEN] = {0};
	...
	printf("sendtoWMH: %u %u %u\n", s_f_cnt, sum, cnt + 1);
		...
		encode_ret = encode_internal_message(INFO_WMH_UDM, tmp_buf, tx_buf);
		if(encode_ret){
			v2x_pr(LOG_LEVEL_ERROR, UDM_V_LOG_ID, "code_internal_message INFO_WMH_UDM err");
		}
